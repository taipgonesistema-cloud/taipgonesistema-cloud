<p align="center">
  <img width="100%" src="https://capsule-render.vercel.app/api?type=waving&height=220&color=0:000000,100:000000&text=denkai_urahara&fontColor=ff1f1f&fontSize=42&fontAlignY=45&animation=fadeIn" alt="Black and red profile banner" />
</p>

<p align="center">
  <img src="https://media.giphy.com/media/3o7abB06u9bNzA8lu8/giphy.gif" width="230" alt="Anakin Skywalker GIF" />
</p>

<h1 align="center">The code is strong with this one.</h1>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Orbitron&weight=700&size=22&pause=900&color=F97316&center=true&vCenter=true&width=720&lines=denkai;Fullstack+Developer;Cloud+%26+Automation;WhatsApp+Systems;Dashboards+and+APIs" alt="Typing SVG" />
</p>

<p align="center">
  <a href="https://github.com/taipgonesistema-cloud?tab=repositories">
    <img src="https://img.shields.io/badge/Repositories-111111?style=for-the-badge&logo=github&logoColor=white" alt="Repositories" />
  </a>
  <a href="https://github.com/taipgonesistema-cloud/pulse-hub">
    <img src="https://img.shields.io/badge/Pulse%20Hub-f97316?style=for-the-badge&logo=whatsapp&logoColor=white" alt="Pulse Hub" />
  </a>
  <img src="https://img.shields.io/badge/Dark%20Side-7f1d1d?style=for-the-badge&logo=sith&logoColor=white" alt="Dark Side" />
</p>

---

## About Me

Sou desenvolvedor focado em criar sistemas praticos, escalaveis e prontos para producao. Trabalho com aplicacoes web, APIs, automacoes, dashboards, integrações e infraestrutura cloud.

No momento estou construindo o **Pulse Hub**, uma central de atendimento e CRM omnichannel com WhatsApp, WebSocket, Postgres, Redis, painel em tempo real e deploy containerizado.

```txt
Jedi path:      shipping real products
Current focus:  SaaS, automations, dashboards and APIs
Mindset:        less talk, more deploy
```

## Tech Arsenal

<p align="center">
  <img src="https://skillicons.dev/icons?i=ts,js,react,nextjs,nodejs,nestjs,go,postgres,redis,docker,git,github,linux,tailwind" alt="Tech stack" />
</p>

## Featured Project

<div align="center">
  <a href="https://github.com/taipgonesistema-cloud/pulse-hub">
    <img src="https://img.shields.io/badge/Pulse%20Hub-Open%20Repository-f97316?style=for-the-badge&logo=github&logoColor=white" alt="Pulse Hub repository" />
  </a>
  <br />
  <br />
  <table>
    <tr>
      <td align="center"><strong>Pulse Hub</strong></td>
    </tr>
    <tr>
      <td align="center">Omnichannel CRM with WhatsApp, realtime dashboard, Go API, Next.js, Postgres and Redis.</td>
    </tr>
    <tr>
      <td align="center"><a href="https://github.com/taipgonesistema-cloud/pulse-hub">View repository</a></td>
    </tr>
  </table>
</div>

## GitHub Stats

<div align="center">
  <a href="https://github.com/taipgonesistema-cloud?tab=repositories">
    <img src="https://img.shields.io/badge/Public%20Repos-111111?style=for-the-badge&logo=github&logoColor=white" alt="Public repositories" />
  </a>
  <a href="https://github.com/taipgonesistema-cloud?tab=followers">
    <img src="https://img.shields.io/badge/Followers-7f1d1d?style=for-the-badge&logo=github&logoColor=white" alt="GitHub followers" />
  </a>
  <a href="https://github.com/taipgonesistema-cloud">
    <img src="https://img.shields.io/badge/Profile-f97316?style=for-the-badge&logo=github&logoColor=white" alt="GitHub profile" />
  </a>
</div>

<br />

<div align="center">
  <table>
    <tr>
      <td align="center"><strong>Focus</strong></td>
      <td align="center"><strong>Stack</strong></td>
      <td align="center"><strong>Shipping</strong></td>
    </tr>
    <tr>
      <td align="center">SaaS and automation</td>
      <td align="center">Next.js, Go, Postgres, Redis</td>
      <td align="center">Production-ready apps</td>
    </tr>
  </table>
</div>

## Code Philosophy

> Fear is the path to the dark side. Bad architecture is too.

- Build simple.
- Deploy fast.
- Measure everything.
- Automate repeated work.
- Keep the system maintainable.

---

<p align="center">
  <img width="100%" src="https://capsule-render.vercel.app/api?type=waving&height=120&section=footer&color=0:f97316,50:7f1d1d,100:050505" alt="Footer" />
</p>
